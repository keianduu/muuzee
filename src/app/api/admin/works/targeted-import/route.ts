...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ message: body.saveCandidates ? `Work候補を保存し、Source Policyに従って${result.autoApplied}件を自動反映しました。` : "Work source coverageを確認しました。", ...result, candidateRows: body.saveCandidates ? await listWorkImportCandidates() : [] });
  } catch (error) {
