...
export function MasterList({ entity, result, queryString }: { entity: MasterEntity; result: MasterListResult; queryString: string }) {
  ...
  return <>
    ...
    <div className="master-action-bar">
      ...
      {(entity !== "works" || workView === "adopted") && <><span className="action-spacer"/><button className="button secondary" disabled={busy || !selected.length} onClick={() => bulk("publish")}>選択をPublish（公開）</button><button className="button secondary" disabled={busy || !selected.length} onClick={() => bulk("unpublish")}>選択をUnpublish（非公開）</button></>}
    </div>
