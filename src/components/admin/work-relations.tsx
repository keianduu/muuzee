...
export function WorkRelations({ workId, artists, holdings, presentations }: { workId: string; artists: Relation[]; holdings: Relation[]; presentations: Relation[] }) {
  ...
  async function setVisibility(kind: "artist" | "holding", row: Relation, visibility: "public" | "hidden") {
    setBusy(true); setMessage("");
    try {
      const response = await fetch(`/api/admin/masters/works/${workId}/relations`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind, relationId: row.id, visibility }) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error || "Request failed");
      setMessage(result.message); router.refresh(); window.dispatchEvent(new CustomEvent("muuzee:master-updated"));
    } catch (error) { setMessage(error instanceof Error ? error.message : "Request failed"); } finally { setBusy(false); }
  }
  function relationMeta(row: Relation) {
    return <><small className="muted">Source: {String(row.source || "manual")}{row.source_url ? <> · <a href={String(row.source_url)} target="_blank" rel="noreferrer">Provenance</a></> : ""} · Resolved</small><br/><small className="muted">Visibility: {String(row.visibility_status || "public")}{row.visibility_overridden ? " · Manual override" : " · Source default"}</small></>;
  }
  return <section><h2>Relations</h2><div className="relation-grid">
    <article className="card"><h3>Artists</h3>{artists.map((row) => <div className="relation-row relation-row--managed" key={String(row.id)}><span><strong>{relationName(row, "artists")}</strong> <small className="muted">{String(row.role || "role未設定")}</small><br/>{relationMeta(row)}</span><div className="relation-actions"><label className="visibility-toggle"><input type="checkbox" checked={String(row.visibility_status || "public") === "public"} disabled={busy} onChange={(event) => setVisibility("artist", row, event.target.checked ? "public" : "hidden")}/><span>Public</span></label><button className="button danger" disabled={busy} onClick={() => mutate("DELETE", { kind: "artist", relationId: row.id })}>Remove</button></div></div>)}{!artists.length && <p className="empty-state">Artist relationはありません。</p>}<div className="field"><label>Artist search</label><input onChange={(event) => search("artists", event.target.value)} placeholder="名前で検索"/></div><div className="relation-options">{artistOptions.map((option) => <button className="button secondary" disabled={busy || artists.some((row) => String(row.artist_id) === option.id)} key={option.id} onClick={() => mutate("POST", { kind: "artist", targetId: option.id })}>+ {option.name}</button>)}</div></article>
    <article className="card"><h3>Holding Venues</h3>{holdings.map((row) => <div className="relation-row relation-row--managed" key={String(row.id)}><span><strong>{relationName(row, "venues")}</strong> <small className="muted">{String(row.holding_type || "collection")} / {String(row.inventory_number || "inventory未設定")}</small><br/>{relationMeta(row)}</span><div className="relation-actions"><label className="visibility-toggle"><input type="checkbox" checked={String(row.visibility_status || "public") === "public"} disabled={busy} onChange={(event) => setVisibility("holding", row, event.target.checked ? "public" : "hidden")}/><span>Public</span></label><button className="button danger" disabled={busy} onClick={() => mutate("DELETE", { kind: "holding", relationId: row.id })}>Remove</button></div></div>)}{!holdings.length && <p className="empty-state">Holding Venueはありません。</p>}<div className="field"><label>Venue search</label><input onChange={(event) => search("venues", event.target.value)} placeholder="会場名で検索"/></div><div className="relation-options">{venueOptions.map((option) => <button className="button secondary" disabled={busy || holdings.some((row) => String(row.venue_id) === option.id)} key={option.id} onClick={() => mutate("POST", { kind: "holding", targetId: option.id, holdingType: "collection" })}>+ {option.name}</button>)}</div></article>
