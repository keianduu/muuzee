...
export function MasterFilter({ entity, params }: { entity: MasterEntity; params: Record<string, string> }) {
  return <form className="master-filter">
    ...
    <div className="field"><label htmlFor="source">Source（出典）</label><select id="source" name="source" defaultValue={params.source}><option value="">All（すべて）</option><option value="manual">Manual（手動）</option><option value="official_website">Official Website</option><option value="trusted_api">Trusted API</option><option value="csv_import">CSV Import</option><option value="wikidata">Wikidata</option><option value="art_commons">Art Commons</option></select></div>
    {entity === "venues" && <div className="field"><label htmlFor="match">Wikidata Source（識別子状態）</label><select id="match" name="match" defaultValue={params.match}><option value="">All（すべて）</option><option value="matched">Linked（紐付け済み）</option><option value="selection">Multiple candidates（候補選択が必要）</option><option value="unmatched">No candidate（候補なし）</option></select></div>}
