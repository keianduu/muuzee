/* Muuzee Map Discovery — page filter adapter */
(async () => {
  "use strict";
  ...
  function exhibitionMatches(item){
    const filters = state.filters.exhibition;
    if(filters.status.length && !filters.status.includes(item.status)) return false;
    if(filters.expression.length && !filters.expression.includes(item.expressionCategory)) return false;
    return true;
  }

  function museumMatches(item){
    const filters = state.filters.museum;
    ...
    if(filters.area.length){
      ...
      if(!filters.area.includes(value)) return false;
    }

    return true;
  }

  function filterItems({mode,items}){
    return mode === "museum"
      ? items.filter(museumMatches)
      : items.filter(exhibitionMatches);
  }

  function summaryLabels(mode){
    if(mode === "exhibition"){
      const filters = state.filters.exhibition;
      return [
        ...filters.status.map(value => value === "now" ? "開催中" : "開催予定"),
        ...filters.expression
      ];
    }
  ...
  function renderFilterUI(){
    ...
    if(exhibitionGroups) exhibitionGroups.hidden = draftMode !== "exhibition";
    if(museumGroups) museumGroups.hidden = draftMode !== "museum";
  ...
  applyButton?.addEventListener("click",() => {
    ...
    discovery.setMode(draftMode,{fit:true});
    renderSummary(discovery.getMode());
