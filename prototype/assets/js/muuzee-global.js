...
(() => {
  ...
  /* discovery-surface-loader:start */
  const discoveryPages = new Set(["index.html","exhibition.html","artist.html","museum.html"]);

  function currentPrototypePage(){
    return location.pathname.split("/").pop() || "index.html";
  }

  function loadSharedScript(filename,ready){
    if(typeof ready === "function" && ready()) return Promise.resolve();

    const absolute = new URL(filename,sharedJsBaseUrl).href;
    const existing = [...document.scripts].find(script => script.src && script.src.split("?")[0] === absolute.split("?")[0]);
    if(existing){
      if(existing.dataset.muuzeeLoaded === "true") return Promise.resolve();
      return new Promise((resolve,reject) => {
    ...
    return new Promise((resolve,reject) => {
      ...
      node.async = false;
      node.onload = () => {
        node.dataset.muuzeeLoaded = "true";
        resolve();
      };
      node.onerror = () => reject(new Error(`shared-script-load-failed:${filename}`));
      document.head.appendChild(node);
    });
  }

  async function initDiscoverySurfaces(){
    if(!discoveryPages.has(currentPrototypePage())) return;

    await loadSharedScript("artist-data.js?v=20260919-order656-01",() => Array.isArray(window.MuuzeeArtistCatalog));
    await loadSharedScript("museum-data.js?v=20260914-surface-01",() => Array.isArray(window.MuuzeeMuseumCatalog));
    ...
    await loadSharedScript("muuzee-surface-fixtures.js?v=20260919-order656-02",() => Boolean(window.MuuzeeSurfaceFixturesReady));
    ...
    await loadSharedScript("muuzee-surface-runtime.js?v=20260919-order656-01",() => Boolean(window.MuuzeeSurfaceRuntimeReady));
  }
  ...
  */
  ...
  function initHamburgerNavigation(){
    ...
    const loggedIn = Boolean(window.MuuzeeAuth?.isLoggedIn?.());
    const menuItems = [
      ...
      ...(loggedIn
        ...
        : [{label:"ログイン / 新規登録",action:"login",auth:true,authFirst:true}]),
    ...
    toggle.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 7h14M5 12h14M5 17h14"></path></svg>';
    ...
    noticeCta.innerHTML = `
      ...
      <span class="muuzee-menu-notice-copy"><strong>お知らせ</strong><small>最新のお知らせを確認</small></span>
    ...
    menuItems.forEach(item => {
      ...
      const link = document.createElement(isAction ? "button" : "a");
      ...
      if(isAction){
        link.type = "button";
        if(isLoginAction){ link.classList.add("is-login"); link.dataset.muuzeeLoginTrigger = ""; }
        if(isLogoutAction){ link.classList.add("is-logout"); link.dataset.muuzeeLogoutTrigger = ""; }
      }else{
        link.href = item.href;
        const hrefPath = item.href.split("#")[0].replace("./","");
        if(hrefPath === currentPath && !item.href.includes("#")){
    ...
    function setOpen(next){
      ...
      if(open){ window.requestAnimationFrame(() => noticeCta.focus({preventScroll:true})); }
    ...
    drawer.addEventListener("click",event => {
      if(event.target.closest("a,button[data-muuzee-login-trigger],button[data-muuzee-logout-trigger]")) setOpen(false);
    });
  ...
  function loadImageMeta(src){
    if(imageMetaCache.has(src)) return Promise.resolve(imageMetaCache.get(src));
    return new Promise(resolve => {
      const img = new Image();
      img.onload = () => {
        const meta = {ratio:(img.naturalHeight || 1) / (img.naturalWidth || 1)};
  ...
  async function layoutMasonry({
    grid,items,getSrc = item => item.src,getRatio = item => item?.aspectRatio ?? item?.ratio,
    renderItem,columns = 4,gapDesktop = 8,gapMobile = 4,minRatio = .58,maxRatio = 1.85
  }){
    if(!grid || !Array.isArray(items) || !items.length || typeof renderItem !== "function") return;
    ...
    const metas = knownMetas.every(Boolean)
      ? knownMetas
      : await Promise.all(knownMetas.map((meta,index) => meta || loadImageMeta(getSrc(items[index]))));
    ...
    items.forEach((item,index) => {
      ...
      const geometry = {
        left:Math.round(column * (columnWidth + gap)),top:Math.round(columnHeights[column]),
        width:Math.round(columnWidth),height:itemHeight,column
      };
  ...
  window.Muuzee.getArtWallImageData = src => window.MuuzeeArtWallImageData?.[src] || null;
  ...
  function syncHeaderAuthUI(){
    ...
    login.title = "ログイン";
    login.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.2"></circle><path d="M5.5 20c.7-4.1 3-6.2 6.5-6.2s5.8 2.1 6.5 6.2"></path></svg>';
  ...
  async function initGlobalUI(){
    ...
    initHeaderReveal();
    initHamburgerNavigation();
    initDiscoverySurfaces().catch(error => console.warn("Muuzee discovery bootstrap failed",error));

    window.addEventListener("muuzee:auth-change",handleAuthStateChange);
  }
