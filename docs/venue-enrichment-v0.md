...
Image Discoveryは確定済みWikidata QIDをidentityとして再利用し、過去の`no_entity_candidate` / `no_image_candidate`を最終結果として扱わない。探索順はWikidata P18、Wikimedia Commons Category（P373）、Wikipedia Article Imagesで、同じCommons File Titleを重複排除したうえでVenueあたり最大3件に制限する。P18は常に先頭候補とし、Category / Article画像はlogo、icon、map、diagram、SVG、poster、painting、sculpture、artwork等を除外し、施設名またはbuilding / exterior / interior / entrance / gallery / museum等を含む画像を優先する。
